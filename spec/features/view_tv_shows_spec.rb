...
feature "user views list of TV shows" do
  ...
  scenario "view details for a TV show" do
      visit "/television_shows/7"

      expect(page).to have_content("Scrubs Network: Comedy Central Years: 2004 - 2012 Genre: Comedy
      Medical Comedy involving both real world medical issues and slapstick comedy antics
      Back to all shows")
  end

  scenario "view details for a TV show with missing information" do
    visit "/television_shows/13"

    expect(page).to have_content("Network: HBO Years: 2011 - Genre: Fantasy")
  end
end
